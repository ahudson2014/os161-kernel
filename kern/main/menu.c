...
cmd_progthread(void *ptr, unsigned long nargs)
{
	...
	result = runprogram_args(progname, nargs, args);                
	if (result) {
...
static
void
cmd_progthread2(void *ptr, unsigned long nargs)
{
	char *cmd = ptr;
	kprintf("cmd = %s\n", cmd); 
	...
	char *argv[MAXMENUARGS];
	nargs=0;
	char *word;
	char *context;	

	//kprintf("I am in cmd_progthread2\n");
...
common_prog(int nargs, char **args)
{
	...
	result = aux_thread_fork(args[0] /* thread name */,
...
static
int
common_prog2(int nargs, char *cmd, char*name)
{
	int result;
	...
	kprintf("Warning: this probably won't work with a "
		...
		//kprintf("I am in comon_prog2, cmd = %s\n", cmd);

	result = aux_thread_fork(name /* thread name */,
			cmd /* thread arg */, nargs /* thread arg */,
			cmd_progthread2, NULL);         
...
static
int
cmd_prog2(int nargs, char *cmd, char* name)
{
	if (nargs < 1) {
		kprintf("Usage: p program [arguments]\n");
		return EINVAL;
	...
	//kprintf("I am in cmd_prog2, cmd = %s\n", cmd);
	//return common_prog(nargs, args);
	return common_prog2(nargs, cmd, name);
}

/*
 * Command for running hungry birds.
 */
static
int
cmd_hungrybirds()
...
cmd_opsmenu(int n, char **a)
{
	...
	showmenu("OS/161: Operations", opsmenu);
	return 0;
}
...
cmd_dispatch(char *cmd)
{
	...
	//kprintf("here, cmd=%s\n", cmd);		
	char* bup_cmd = (char *)kmalloc(sizeof(char *)*strlen(cmd));
	bup_cmd = kstrdup(cmd);
	...
	for (i=0; cmdtable[i].name; i++) {
		if (*cmdtable[i].name && !strcmp(args[0], cmdtable[i].name)) {
			...
			if(i==6 && nargs>2)
			{
				//kprintf("I am here, cmd=%s\n", bup_cmd);					
				result = cmd_prog2(1, bup_cmd, args[1]);
			}
			else
...
menu(char *args)
{
	...
	while (1) {
		...
		//total_tlb_faults = total_page_faults = total_asyncpage_write = 0;
		menu_execute(buf, 0);  
		//kprintf("tlb_faults: %d, page_faults: %d, async_writes: %d\n", total_tlb_faults, total_page_faults, total_asyncpage_write);              
	}
}
