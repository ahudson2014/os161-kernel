...
vfs_setbootfs(const char *fsname)
{
	...
	DEBUG(DB_VFS, "Setting bootfs to %s in vfslookup.c.\n", fsname);
	       ...
	       char *buf, size_t buflen)
{
	...
	if (result) {
		//kprintf("Couldn't get device in vfs_lookparent!\n");
		return result;
	}
	//kprintf("New path after getdevice = %s\n", path);
	if (strlen(path)==0) {
		/*
		 ...
		 * operation.
		 */
		//kprintf("path length = 0 in vfs_lookparent!\n");
		result = EINVAL;
	}
	else {
		result = VOP_LOOKPARENT(startvn, path, retval, buf, buflen);
		if (result){
			//kprintf("VOP_LOOKPARENT failed in vfs_lookparent!\n");
		return result;
		}
	}
	//kprintf("vfs_lookparent succesful!\n");
	VOP_DECREF(startvn);
	return result;
}
...
vfs_lookup(char *path, struct vnode **retval)
{
	...
	//kprintf("vfs_lookup looking for path: %s\n", path);

	result = getdevice(path, &path, &startvn);
	if (result) {
		//kprintf("Couldn't get device in vfs_lookup!\n");
		return result;
	}
