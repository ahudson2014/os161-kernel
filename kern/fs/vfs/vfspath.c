...
vfs_open(char *path, int openflags, struct vnode **ret)
{
	...
	if (openflags & O_CREAT) {
		...
		if (result) {
			//kprintf("vfs_lookparent failed in vfs_open.\n");
			return result;
		}
	...
	if (result) {
		//kprintf("vfs_lookup failed in vfs_open.\n");
		return result;
	}
	...
	if (result) {
		VOP_DECREF(vn);
		//kprintf("VOP_OPEN failed in vfs_open.\n");
		return result;
	}
...
vfs_remove(char *path)
{
	...
	result = vfs_lookparent(path, &dir, name, sizeof(name));
	if (result) {
		//kprintf("vfs_lookparent inside vfs_remove failed!\n");
		return result;
	}

	result = VOP_REMOVE(dir, name);
	if (result){
		//kprintf("VOP_REMOVE failed in vfs_remove!\n");
		return result;
	}
	VOP_DECREF(dir);
...
vfs_mkdir(char *path)
{
	...
	if (result) {
		//kprintf("vfs_lookparent inside vfs_mkdir failed!\n");
		return result;
	}

	//added by tocurtis
	if(parent == NULL){
		//kprintf("vfs_lookparent returned a NULL node!\n");
		return result;
	}
	...
	if (result){
		//kprintf("VOP_MKDIR failed in vfs_mkdir!\n");
		return result;
	}
...
vfs_rmdir(char *path)
{
	...
	result = vfs_lookparent(path, &parent, name, sizeof(name));
	if (result) {
		//kprintf("Could not find parent in vfs_rmdir!\n");
		return result;
	}

	result = VOP_RMDIR(parent, name);
	if(result){
		//kprintf("VOP_RMDIR failed!\n");
		return result;
	}
