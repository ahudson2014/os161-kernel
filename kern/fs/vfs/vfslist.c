...
vfs_bootstrap(void)
{
	...
	//Added by rahmanmd
	fs_lock = lock_create("fs_lock");
	if (fs_lock==NULL) {
		panic("vfs: Could not create filesystems global lock\n");
	}
	kprintf("global lock created\n");
...
vfs_getroot(const char *devname, struct vnode **result)
{
	...
	num = array_getnum(knowndevs);
	DEBUG(DB_VFS, "Getroot: Found %u devices.\n", num);
	for (i=0; i<num; i++) {
		...
		if (kd->kd_fs!=NULL) {
			...
			if (!strcmp(kd->kd_name, devname) ||
				...
				*result = FSOP_GETROOT(kd->kd_fs);
				DEBUG(DB_VFS, "Getroot: Device found and mounted.\n");
				goto out;
			}
		}
		else {
			if (kd->kd_rawname!=NULL &&
				...
				err = ENXIO;
				DEBUG(DB_VFS, "Getroot: Device not found.\n");
				goto out;
			}
		}
		...
		if (!strcmp(kd->kd_name, devname)) {
			...
			*result = kd->kd_vnode;
			DEBUG(DB_VFS, "Getroot: Device found but is not mountable.\n");
			goto out;
		}
		...
		if (kd->kd_rawname!=NULL && !strcmp(kd->kd_rawname, devname)) {
			...
			VOP_INCREF(kd->kd_vnode);
			*result = kd->kd_vnode;
			DEBUG(DB_VFS, "Getroot: Device has rawname--returning device itself.\n");
			goto out;
		}
	 ...
	 */
	DEBUG(DB_VFS, "Getroot: Device specified by devname doesn't exist.\n");
	err = ENODEV;
...
vfs_getdevname(struct fs *fs)
{
	...
	num = array_getnum(knowndevs);
	DEBUG(DB_VFS, "Getdevname: Found %u devices.\n", num);
...
findmount(const char *devname, struct knowndev **result)
{
	...
	num = array_getnum(knowndevs);
	DEBUG(DB_VFS, "Findmount: Found %u devices.\n", num);
