...
#define BUF_CACHE_REP_ALGO RND

/*
 * Added by rahmanmd
 * 
 * The buffer cache is an array of 128 disk block of size 512 bytes 
 * each. Total memory needed to store the cache is 
 * 128*(4+4+4+512) = 66 KB which is reasonable
 */
static struct sfs_buffer_cache buffer_cache[BUF_CACHE_SIZE];	
static struct lock *sfs_buffer_cache_lock;
...
sfs_init_buffer_cache()
{
	...
	{
		buffer_cache[i].sfs_inode_no = -1;
		buffer_cache[i].disk_block_no = -1;
		buffer_cache[i].last_access_time = 0;								
	}

	//block lock initializing, added by rahmanmd
	sfs_buffer_cache_lock = lock_create("block lock");
        if(sfs_buffer_cache_lock == NULL) 
        {
		return ENOMEM;
	}
...
static int 
sfs_get_buffer_cache(int sfs_inode_no, int disk_block_no, char* buf)
{
	int i, result;	

	lock_acquire(sfs_buffer_cache_lock);

	//round back the access time counter
	if(access_time == 65535)
	{
		access_time = 0;
	}
	else access_time++;
	...
	for(i=0;i<BUF_CACHE_SIZE; i++)
	{				
		if(buffer_cache[i].sfs_inode_no == sfs_inode_no && 
		   buffer_cache[i].disk_block_no == disk_block_no)
		{
			buffer_cache[i].last_access_time = access_time;

			strcpy(buf, buffer_cache[i].buffer_cache);						
			lock_release(sfs_buffer_cache_lock);
			return 0;
		}

		//round back each of the last access time counter
		if(access_time == 0)
			buffer_cache[i].last_access_time = 0;
	}
...
static int 
sfs_get_free_slot(int *free_slot)
{
	int i;

	for(i=0;i<BUF_CACHE_SIZE; i++)
	{
		if(buffer_cache[i].sfs_inode_no == -1)
		{
			*free_slot = i;
			return 0;
		}
...
static int 
sfs_replace_lru_cache(struct sfs_fs *sfs, int *victim_slot)
{
	int i, result;
	struct sfs_buffer_cache victim_cache;
	
	int oldest_time = 65536;
	int oldest = -1;
	//get the LRU victim
	for(i=0;i<BUF_CACHE_SIZE; i++)
	{
		if(buffer_cache[i].last_access_time < oldest)
		{
			oldest = i;
			oldest_time = buffer_cache[i].last_access_time;
		}
	}
	victim_slot = oldest;
	victim_cache = 	 buffer_cache[*victim_slot];

	//Write back the victim cache buffer to disk
	result=sfs_wblock(sfs, victim_cache.buffer_cache, victim_cache.disk_block_no);
...
static int 
sfs_replace_random_cache(struct sfs_fs *sfs, int *victim_slot)
{
	int result;
	struct sfs_buffer_cache victim_cache;

	//get a random victim
	victim_slot = (random() % BUF_CACHE_SIZE);
	victim_cache = 	 buffer_cache[*victim_slot];
...
static int 
sfs_add_buffer_cache(struct sfs_fs *sfs, int inode_no, int disk_block_no, int *free_slot)
{
	int i, result;
	...
	//get a free slot
	result = sfs_get_free_slot(&free_slot);
	//if no free slot then replace a victim slot	
	if(result)
	{
		switch(BUF_CACHE_REP_ALGO)
		{
			case RND:
			{
				result = sfs_replace_random_cache(sfs, &replacement_slot);			
				break;
			}
			case LRU:
			{
				result = sfs_replace_lru_cache(sfs, &replacement_slot);
				break;
			}
			default:
			{
	...
	if(free_slot==-1 && replacement_slot == -1)
	{
		lock_release(sfs_buffer_cache_lock);
		kprintf("bad stuff!");
		return ENOMEM;
	...
	//add the entry
	access_time++;
	buffer_cache[replacement_slot].sfs_inode_no=inode_no;
	buffer_cache[replacement_slot].disk_block_no=disk_block_no;
	buffer_cache[replacement_slot].last_access_time = access_time;

	result=sfs_rblock(sfs, buffer_cache[replacement_slot].buffer_cache, disk_block_no);
	if(result)
	{
		buffer_cache[replacement_slot].sfs_inode_no = -1;
		lock_release(sfs_buffer_cache_lock);
		return result;
	}		
	      ...
	      u_int32_t skipstart, u_int32_t len)
{
	...
	if (diskblock == 0) {
		...
		// handling buffer cache
		//
		//first check the cache, if it is there then return if not then read it from disk
		//and add this block to cache (replce a cache if the cache tablis full)
		
		result=sfs_get_buffer_cache(sv->sv_ino, diskblock, iobuf);
		if (result == ENOENT) //not found..bring it back and add it to cache
		{
			result = sfs_add_buffer_cache(sfs, sv->sv_ino, diskblock, &free_slot);
			if(!result && free_slot!=-1)
				return result;
		}
		else
		{
			//found in the cache..get it into the buffer
			result= uiomove(buffer_cache[free_slot].buffer_cache+skipstart, len, uio);			
			return result;
		}
		    ...
		    u_int32_t *ino, int *slot, int *emptyslot)
{
	...
	if (!found && !strcmp(name, "."))
	{
		//kprintf("Returning root directory\n.");
		if (nentries==0)
		{
			// Make an initial entry here?
			//kprintf("No directory entries on root.\n");
		}
		else
		{
 ...
 * Added by rahmanmd
 *
 * filesystem implementation of getdirentry. Basically go through the sfs_dir
 * entries of a directory and read the directory at the offset specified. This
 * is basically same as sfs_dir_findname except that we try tp find a valid entry
 * from the offset in the directory. 
 *
 * MOST OF THE CODES ARE TAKEN FROM sfs_dir_findname
 */
int
sfs_getdirentry(struct vnode *v, struct uio *uio)
{
	...
	/*check id the offset is out of the size of this directory*/
	if(uio->uio_offset >= nentries) {
		splx(spl);
		return ENOENT;
	}
 

	int offset = uio->uio_offset;	

	/* For each slot...starting from the offset specified... */
	for (i=uio->uio_offset; i<nentries; i++) {
		...
		/*If we find a valid entry then break and return the name*/
		if (tsd.sfd_ino != SFS_NOINO) {
			found = 1;	
			kprintf("%s \n", tsd.sfd_name);
			//break;
	...
	/*We found an entry, cpy the name to buffer*/
	result = uiomove(tsd.sfd_name, strlen(tsd.sfd_name), uio);
	uio->uio_offset++;
	//kprintf("%s ", tsd.sfd_name);
 ...
 * Added by rahmanmd
 *
 * Find an inode in the current directory and return its name
 * works same as sfs_dir_findname. 
 * 
 * MOST OF THE CODES ARE TAKEN FROM sfs_dir_findname
 */
static
int
sfs_dir_findinode(struct sfs_vnode *sv, int inode, char *name)
{
	struct sfs_dir tsd;
	int found = 0;
	int nentries = sfs_dir_nentries(sv);
	...
	for (i=0; i<nentries; i++) {
		...
		if(tsd.sfd_ino == inode) {
			/* Ensure null termination, just in case */
			tsd.sfd_name[sizeof(tsd.sfd_name)-1] = 0;
			strcpy(name,tsd.sfd_name);
			return 0;
		}
	}
 ...
 * Added by rahmanmd
 *
 * Get the full pathname for a file. This can handle subdirectories.
 * go up froom the given file up to the device root location.
 */
...
sfs_namefile(struct vnode *vv, struct uio *uio)
{
	...
	struct sfs_vnode *parent = vv->vn_data;
	struct vnode *root_vnode = sfs_getroot(&sfs->sfs_absfs);	
	struct sfs_vnode *sv;
	char cur_dir[] = ".";
	char parent_dir[] = "..";
	char name[256];
	...
	//start from the file and track back parent upto root node
	for(sv = vv->vn_data; &sv->sv_v != root_vnode; sv = parent, inode = sv->sv_ino)
	{
		int result = sfs_dir_findname(sv, parent_dir, &inode, NULL, NULL);
		...
		/*load the parent*/
		result = sfs_loadvnode(sfs, inode, SFS_TYPE_INVAL, &parent);
		if (result) {
			return result;
		}
	
		/*Get the name of this inode and save it to the path*/		
		result = sfs_dir_findinode(parent, sv->sv_ino, name);
		if (result!=0) {
	        	return result;
		}
	...
	save = (char *)kmalloc(1+sizeof(path)+1);
	save[0] = '/';
	strcpy(save+1, path);
	save[strlen(save)] = NULL;
	//kprintf("namefile:%s\n",save);

	int result = uiomove(save, strlen(save), uio);
	if (result) {
		kfree(save);
		return result;
	}
 ...
 * Added by rahmanmd
 *
 * filesystem implementation of mkdir. Create a directory in the current  
 * working directory. If a file exists with the same name then fails.
 * Also properly link the new directory to its parent directory and the
 * curdir entry (.). 
 * 
 * This operation is similar to creating a file using sfs_create() except 
 * that the file type is directory type and need to associate 
 * the new dir with the parent directory for implementing hierarchical 
 * fs.
 *  
 * MOST OF THE CODES ARE TEKEN FROM SFS_CREATE().
 */
static
int
sfs_mkdir(struct vnode *v, const char *dir_name)
{
	struct sfs_fs *sfs = v->vn_fs->fs_data;
	struct sfs_vnode *sv = v->vn_data;
	struct sfs_vnode *newguy;
	...
	/* Look up the name */
	result = sfs_dir_findname(sv, dir_name, &ino, NULL, NULL);
	if (result!=0 && result!=ENOENT) {
		splx(spl);
		return result;
	}
	...
	/* Didn't exist - create it (note: we are creating a directory, so use proper sfs file type) */
	...
	/* Link it into the directory */
	result = sfs_dir_link(sv, dir_name, newguy->sv_ino, NULL);
	if (result) {
		splx(spl);
		//VOP_DECREF(&newguy->sv_v);//its a directory
		return result;
	}

	/* Link the current directory (self vnode link) with its '.' entry*/
	char cur_dir[] = ".";
	result = sfs_dir_link(newguy, cur_dir, newguy->sv_ino, NULL);
	if (result) {
		splx(spl);
		//VOP_DECREF(&newguy->sv_v);//its a directory
		return result;
	...
	/* Link the parent directory with its ".." dir entry*/
	char parent_dir[] = "..";
	result = sfs_dir_link(newguy, parent_dir, sv->sv_ino, NULL);
	if (result) {
		splx(spl);
		//VOP_DECREF(&newguy->sv_v);//its a directory
		return result;
...
sfs_remove(struct vnode *dir, const char *name)
{
	...
	//else kprintf("dir unlink failed\n");
...
/*
 * Added by rahmanmd
 * Checking for directory. No lock used.
 * Assumed that the caller has the lock on fs
 */
int 
sfs_is_dir(struct vnode *v)
{
	struct sfs_vnode *sv = v->vn_data;
...
/*
 * Added by rahmanmd
 * Checking for directory is empty. No lock used
 * Assumed that the caller has the lock on fs
 */
int 
sfs_isempty(struct vnode *v)
{
	struct sfs_vnode *sv = v->vn_data;
	struct sfs_dir dir_entry;
	int result;
	int i;	
	char curdir[] = ".";
	char parentdir[] = "..";
	...
	if((result=sfs_is_dir(v)))
		return result;
	
	int nentries = sfs_dir_nentries(sv);

	/*
	 * Go through directory entries to see whether there
	 * is a file there in this directory. we shouldn't count
	 * the '.' and '..' entries.
	 */
	for(i=0;i<nentries;i++)
	{
		result = sfs_readdir(v, &dir_entry, i);
		if (result)		
			return result;	

		if (dir_entry.sfd_ino != SFS_NOINO && 
			strcmp(dir_entry.sfd_name, curdir) &&
 			strcmp(dir_entry.sfd_name, parentdir)) {
			return ENOTEMPTY;					 
		}
	}
 ...
 * Added by rahmanmd
 *
 * filesystem implementation of rmdir. remove the directory entry in the 
 * current working directory. If the file doesn't exists then fail.
 * Also properly unlink the directory Ffrom its parent directory. Also
 * we make sure that non-empty directory is not deleted.
 * 
 * This operation is similar to removing a file using sfs_remove() except 
 * that the file type is directory type and need to associate 
 * the new dir with the parent directory for implementing hierarchical 
 * fs. 
 *
 * SO MOST OF THE CODES ARE TEKEN FROM SFS_REMOVE().
 */
static
int
sfs_rmdir(struct vnode *dir, const char *name)
{
	struct sfs_vnode *sv = dir->vn_data;
	struct sfs_vnode *victim;
	int slot;
	...
	/* Look for the directory in the parent directory entry and fetch a vnode for it. */
	result = sfs_lookonce(sv, name, &victim, &slot);
	if (result) {
		splx(spl);
		return result;
...
/*
 * Rename a file. (Modified by rahmanmd to incorporate subdirectories)
 *
 * Now, we are implementing subdirectories. So, we the two directories
 * passed may not be same. If the rename operation is moving a directory
 * into another then unlink with the old parent and link with new parent.
 *
 */
static
int
	   ...
	   struct vnode *d2, const char *n2)
{
	...
	//commented by rahmanmd, we have subdirectories now
	//assert(d1==d2);
	...
	//get lock on each of the directories.
	if(d1==d2)
		lock_acquire(d1->vn_dirlock);
	else
	{
		lock_acquire(d1->vn_dirlock);
		lock_acquire(d2->vn_dirlock);
	}
...
/************************************************************************************************/
	/*
	 * Added by rahmanmd for handling subdirectory (move)
	 * 
	 * If the original file is a directory and we are moving	 
	 * this directory into another directory then unlink this
	 * directory from the current parent directory and then
	 * link it with the new parent directory d2.
	 */
	if(g1->sv_i.sfi_type == SFS_TYPE_DIR && d1!=d2)
	{
		struct sfs_vnode *old_parent;
		char parent_dir[] = "..";
		int old_parent_slot;

		/* Look up the old parent of the dir and get its inode and slot number*/
		result = sfs_lookonce(g1, parent_dir, &old_parent, &old_parent_slot); 
		if(result)
		{
			goto puke;
		}

		/*unlink with the old parent*/
		result = sfs_dir_unlink(g1, old_parent_slot);
		if (result) 
		{
			goto puke_harder;
		}
		...
		result = sfs_dir_link(g1, parent_dir, sv2->sv_ino, NULL);
		if (result) 
		{
			goto puke;
		}
...
/**************************************************************************************************/
	...
	if(d1==d2)
		lock_release(d1->vn_dirlock);
	else
	{
		lock_release(d1->vn_dirlock);
		lock_release(d2->vn_dirlock);
	}
...
/* Added by rahmanmd
 * Get the subpath of the top directory in the path
 */
int sfs_get_topdirpath(char *path, char *top_dirpath)
{
	int i=0;
	for(i=0; path[i]!='/' && path[i]!='\0'; i++)
		top_dirpath[i]=path[i];
...
/*
 * Added by rahmanmd
 * lookup every subdirectory in the path and get the final vnode
 */
int
sfs_lookup_subdir(struct sfs_vnode *sv, char *path, struct vnode **ret)
{
	struct sfs_vnode *final;
	int result, offset=0;
	char top_dirpath[256];

	while(offset>=0)
	{
		offset = sfs_get_topdirpath(path, top_dirpath);
		//kprintf("\ntemppath: %s, subpath: %s\n",path, top_dirpath);
		
		/*look for each of the subdirs*/
		result = sfs_lookonce(sv, top_dirpath, &final, NULL);
		if (result) {			
			return result;
		}
		...
		path = path+offset;//next top directory path
		sv=final;//look into subdir vnode	
	}
...
/*
 * Lookup gets a vnode for a pathname. (modified by rahmanmd to support subdir)
 * 
 */
static
int
...
/*****************************************************************************************/
	 ...
	 * Added by rahmanmd
	 * 
	 * If it's subdir then lookup every subdirectory in the path
	 */
	//kprintf("l: before path = %s\n", path);
	char *top_parent_path = strchr(path, '/');	
	//kprintf("l: after path = %s, top= %s\n", path, top_parent_path);
	if(top_parent_path!=NULL)
	{	
		//kprintf("\npath: %s\n",path);
		result=sfs_lookup_subdir(sv, temp_path, ret);
		splx(spl);
		return result;
	}
/*****************************************************************************************/

	//otherwise it's easy - just look up the name.
...
/*
 * Modified by rahmanmd for incorporating subdirectory support
 * 
 * lookparent returns the last path component as a string and the
 * directory it's in as a vnode.
 *
 * Now, we support subdirectories. So, find the last parent path 
 * and do a lookup for that path to get the vnode of the pathname.
 */
static
int
...
/*****************************************************************************************/
	 ...
	 * Added by rahmanmd
	 * 
	 * find the top parent path and do a subdir lookup for that 
	 * path to get the vnode of the final path.
	 */
	//kprintf("lp: before path = %s\n", path);
	//char *top_parent_path = strchr(path, '/');

	int i=0;
	int index = -1;
	while(path[i] && (index = path[i] == '/'?i:index))i++;
	//kprintf("lp: index = %d", index);

	//kprintf("lp: after path = %s\n", path);
	//if(top_parent_path!=NULL)
	if(index!=-1)
	{
		//*top_parent_path = '\0';
		//top_parent_path++;// get the subpath for parent
		//strcpy(buf, top_parent_path);
		strcpy(buf, path+index+1);
		path[index] = '\0';

		//kprintf("lp: returning buf = %s, path = %s\n", buf, path);
		splx(spl);
		result=sfs_lookup(&sv->sv_v, path, ret);
		return result;
	}
/*****************************************************************************************/

	//otherwise just return the root dir and copy the path.
	strcpy(buf, path);
	//kprintf("lp: returning buf (null) = %s, path = %s\n", buf, path);
	VOP_INCREF(&sv->sv_v);
	*ret = &sv->sv_v;
